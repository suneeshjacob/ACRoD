    ...
    for j in range(len(i_current)):
        ...
        if current_joint_temp == 1:
            if f'thd{current_index_reverse_temp}' in symbolicvariableslist:
                velolist_temporary.append(f'-thd{current_index_reverse_temp}*n{current_index_temp}.cross(a-r{current_index_temp})')
                ...
                velolist_temporary.append(f'thd{current_index_temp}*n{current_index_temp}.cross(a-r{current_index_temp})')
                latextemp.append(r'\dot{\theta}_{'+current_index+r'} \hat{n}_{'+current_index+r'} \times \left( \vec{a} - \vec{r}_{'+current_index+r'} \right)')
            ...
            if f'dd{current_index_reverse_temp}' in symbolicvariableslist:
                velolist_temporary.append(f'-dd{current_index_reverse_temp}*n{current_index_temp}')
                ...
                velolist_temporary.append(f'dd{current_index_temp}*n{current_index_temp}')
                latextemp.append(r'\dot{d}_{'+current_index+r'} \hat{n}_{'+current_index+r'}')
        elif current_joint_temp == 3:
            if f'thd{current_index_reverse_temp}' in symbolicvariableslist and f'dd{current_index_reverse_temp}' in symbolicvariableslist:
                velolist_temporary.append(f'-thd{current_index_reverse_temp}*n{current_index_temp}.cross(a-r{current_index_temp})-dd{current_index_reverse_temp}*n{current_index_temp}')
                latextemp.append(r'-\dot{\theta}_{'+current_index_reverse+r'} \hat{n}_{'+current_index+r'} \times \left( \vec{a} - \vec{r}_{'+current_index+r'} \right) - \dot{d}_{'+current_index_reverse+r'} \hat{n}_{'+current_index+r'}')
                angulist_temporary.append(f'-thd{current_index_reverse_temp}*n{current_index_temp}')
                latexomegatemp.append(r'-\dot{\theta}_{'+current_index_reverse+r'} \hat{n}_{'+current_index+r'}')
            else:
                for j2 in ['x','y','z']:
                ...
                tobeexecuted.append(f'dd{current_index_temp}=sympy.symbols(\'\\dot{{d}}_{{{current_index}}}\')')
                symbolicvariableslist.append(f'dd{current_index_temp}')
                velolist_temporary.append(f'thd{current_index_temp}*n{current_index_temp}.cross(a-r{current_index_temp})+dd{current_index_temp}*n{current_index_temp}')
                latextemp.append(r'\dot{\theta}_{'+current_index+r'} \hat{n}_{'+current_index+r'} \times \left( \vec{a} - \vec{r}_{'+current_index+r'} \right) + \dot{d}_{'+current_index+r'} \hat{n}_{'+current_index+r'}')
                angulist_temporary.append(f'thd{current_index_temp}*n{current_index_temp}')
                latexomegatemp.append(r'\dot{\theta}_{'+current_index+r'} \hat{n}_{'+current_index+r'}')
        elif current_joint_temp == 4:
            if f'w{current_index_reverse_temp}' in symbolicvariableslist:
                velolist_temporary.append(f'-w{current_index_reverse_temp}.cross(a-r{current_index_temp})')
                latextemp.append(r'-\vec{\omega}_{'+current_index_reverse+r'} \times \left( \vec{a} - \vec{r}_{'+current_index+r'} \right)')
                angulist_temporary.append(f'-w{current_index_reverse_temp}')
                latexomegatemp.append(r'-\vec{\omega}_{'+current_index_reverse+r'}')
            else:
                for j2 in ['x','y','z']:
                    tobeexecuted.append(f'w{current_index_temp}{j2}=sympy.symbols(\'\\omega_{current_index}{j2}\')')
                    symbolicvariableslist.append(f'w{current_index_temp}{j2}')
                    tobeexecuted.append(f'r{current_index_temp}{j2}=sympy.symbols(\'r_{{{current_index}{j2}}}\')')
                    symbolicvariableslist.append(f'r{current_index_temp}{j2}')
                tobeexecuted.append(f'w{current_index_temp}=sympy.Matrix([w{current_index_temp}x,w{current_index_temp}y,w{current_index_temp}z])')
                tobeexecuted.append(f'r{current_index_temp}=sympy.Matrix([r{current_index_temp}x,r{current_index_temp}y,r{current_index_temp}z])')
                velolist_temporary.append(f'w{current_index_temp}.cross(a-r{current_index_temp})')
                latextemp.append(r'\vec{\omega}_{'+current_index+r'} \times \left( \vec{a} - \vec{r}_{'+current_index+r'} \right)')
                angulist_temporary.append(f'w{current_index_temp}')
                latexomegatemp.append(r'\vec{\omega}_{'+current_index+r'}')
        elif current_joint_temp == 5:
            if f'thd{current_index_reverse_temp}' in symbolicvariableslist and f'gamd{current_index_reverse_temp}' in symbolicvariableslist:
                velolist_temporary.append(f'-(gamd{current_index_reverse_temp}*m{current_index_temp}+thd{current_index_reverse_temp}*n{current_index_temp}).cross(a-r{current_index_temp})')
                latextemp.append(r'-\left(\dot{\gamma}_{'+current_index_reverse+r'} \hat{m}_{'+current_index+r'}+\dot{\theta}_{'+current_index_reverse+r'} \hat{n}_{'+current_index+r'}\right) \times \left( \vec{a} - \vec{r}_{'+current_index+r'} \right)')
                angulist_temporary.append(f'-(gamd{current_index_reverse_temp}*m{current_index_temp})+thd{current_index_reverse_temp}*n{current_index_temp})')
                latexomegatemp.append(r'-\dot{\gamma}_{'+current_index_reverse+r'} \hat{m}_{'+current_index+r'}+\dot{\theta}_{'+current_index_reverse+r'} \hat{n}_{'+current_index+r'}')
            else:
                for j2 in ['x','y','z']:
                    tobeexecuted.append(f'n{current_index_temp}{j2}=sympy.symbols(\'n_{{{current_index}{j2}}}\')')
                    symbolicvariableslist.append(f'n{current_index_temp}{j2}')
                    tobeexecuted.append(f'm{current_index_temp}{j2}=sympy.symbols(\'m_{{{current_index}{j2}}}\')')
                    symbolicvariableslist.append(f'm{current_index_temp}{j2}')
                    tobeexecuted.append(f'r{current_index_temp}{j2}=sympy.symbols(\'r_{{{current_index}{j2}}}\')')
                    symbolicvariableslist.append(f'r{current_index_temp}{j2}')
                tobeexecuted.append(f'n{current_index_temp}=sympy.Matrix([n{current_index_temp}x,n{current_index_temp}y,n{current_index_temp}z])')
                tobeexecuted.append(f'm{current_index_temp}=sympy.Matrix([m{current_index_temp}x,m{current_index_temp}y,m{current_index_temp}z])')
                tobeexecuted.append(f'r{current_index_temp}=sympy.Matrix([r{current_index_temp}x,r{current_index_temp}y,r{current_index_temp}z])')
                tobeexecuted.append(f'thd{current_index_temp}=sympy.symbols(\'\\dot{{\\\\theta}}_{{{current_index}}}\')')
                symbolicvariableslist.append(f'thd{current_index_temp}')
                tobeexecuted.append(f'gamd{current_index_temp}=sympy.symbols(\'\\dot{{\\gamma}}_{{{current_index}}}\')')
                symbolicvariableslist.append(f'gamd{current_index_temp}')
                velolist_temporary.append(f'(gamd{current_index_temp}*m{current_index_temp}+thd{current_index_temp}*n{current_index_temp}).cross(a-r{current_index_temp})')
                latextemp.append(r'\left(\dot{\gamma}_{'+current_index+r'} \hat{m}_{'+current_index+r'}+\dot{\theta}_{'+current_index+r'} \hat{n}_{'+current_index+r'}\right) \times \left( \vec{a} - \vec{r}_{'+current_index+r'} \right)')
                angulist_temporary.append(f'gamd{current_index_temp}*m{current_index_temp}+thd{current_index_temp}*n{current_index_temp}')
...
def jacobian(A_new):
    ...
    for i in finalconn:
        ...
        #print(locals().keys())
        #eval('thd_1_2')
        velolist_temp_matrix = sympy.zeros(3,1)
        ...
        if i in finalconn2:
            angulist_temp_matrix = sympy.zeros(3,1)
            ...
            angulist.append(sympy.Matrix([sympy.nan for i in range(3)]))
    ...
    for i in velolist+angulist:
        for j in i.free_symbols:
            full_symbols.add(j)
    full_velocities = {i for i in full_symbols if str(i)[0] not in ['a','n','r','m']}
    ...
    if len(issphericalcontrollable_info[1])==0:
        for i in range(len(issphericalcontrollable_info[0])):
            ...
            absolute_velocity_of_a_superfluous_link_str = velosforpath(A,path_to_superfluous_link)[2]
    ...
    number_of_superfluous_equations = len(superfluous_equations)
    ...
    if len(passive_velocities_sorted)>0:
        ...
        if number_of_superfluous_equations!=0:
    ...
    alpha_fun = lambda t: t+sympy.pi*(1-sympy.functions.special.delta_functions.Heaviside(t, H0=0))
    ...
    for i in minds:
        ...
        exec(f'alpha_{tempindsstr[0]}_{tempindsstr[1]}=alpha_fun(sympy.atan(-1/(sympy.tan(beta_{tempindsstr[0]}_{tempindsstr[1]})*sympy.cos(delta_{tempindsstr[0]}_{tempindsstr[1]}-phi_{tempindsstr[0]}_{tempindsstr[1]}))))')
    ...
    return [J1,J2,A1,A2,[active_velocities_sorted,passive_velocities_sorted],[latexstring,latexomegastring],symbolicvariableslist,new_variables,[J1_new,J2_new,A1_new,A2_new],number_of_superfluous_equations]
